    ...
    class Bot extends Common {
        ...
        async processing() { // eslint-disable-line
            ...
            try {
                ...
                Object.keys(shares).forEach(s => {
                    if (!tradeUids.includes(s)) {
                        delete shares[s];
                    }
                });
                ...
                */
                ...
                for (let i = 0; i < topBuyWithDelta.length; i++) {
                    ...
                    if (!instrument?.stat?.buy?.sumPerc || !instrument?.stat?.sell?.sumPerc) {
                        continue;
                    }
